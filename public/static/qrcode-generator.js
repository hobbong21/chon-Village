/**
 * QR Code Generator - Self-contained QR code generation library
...
(function(global) {
  ...
  // Polynomial math for error correction
  class QRPolynomial {
  ...
  class QRCode {
    ...
    makeImpl(test, maskPattern) {
      ...
      if (this.dataCache == null) {
        this.dataCache = QRCode.createData(this.typeNumber, this.errorCorrectLevel, this.text);
      }
    ...
    static createData(typeNumber, errorCorrectLevel, text) {
  ...
  global.QRCodeGenerator = {
    ...
    toCanvas: function(canvas, text, options = {}) {
      const qr = new QRCode(text, {
        ...
        errorCorrectLevel: options.errorCorrectLevel || QRErrorCorrectLevel.M
      });

      const ctx = canvas.getContext('2d');
      const cellSize = Math.floor((options.width || 256) / qr.moduleCount);
      const margin = options.margin || 2;
      
      canvas.width = cellSize * qr.moduleCount + margin * 2 * cellSize;
      canvas.height = cellSize * qr.moduleCount + margin * 2 * cellSize;
